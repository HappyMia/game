...
//a:beta, b=this.angle, t=0.6
/* function lerpAngle(a, b, t) {     //每一次旋转的角度
	var d = b - a;
	if (d > Math.PI) d = d - 2 * Math.PI;  //将d的角度转化在(-pi，pi)
	if (d < -Math.PI) d = d + 2 * Math.PI;
	return a + d * t;
} */
function lerpAngle(a, b, t) {     //每一次旋转的角度，另一种写法
	var d = a - b;
	if (d > Math.PI) d = d - 2 * Math.PI;  //将d的角度转化在(-pi，pi)
	if (d < -Math.PI) d = d + 2 * Math.PI;
	return a + d * t;
...
function lerpDistance(aim, cur, ratio) {   //aim：目标   cur：当前   ratio：百分比     每一次趋近的距离
	...
	var delta = aim - cur;   //另一种写法
	return cur + delta * ratio;
}
function avelerpDistance(aim, cur, ratio){
	var delta = 1000/(aim - cur);   //另一种写法
	return cur + delta * ratio;
}
